...
def main(save_pdf=False, show_plot=True):
    ...
    # overlap, m
    m = ax1.imshow(np.flip(syn_theory_data['mean_m'], axis=0), cmap=cmp, origin='lower', vmin=0.6, vmax=1.)
    ...
    m = ax2.imshow(np.flip(syn_theory_data['mean_alpha_cxt'], axis=0), cmap=cmp, origin='lower', vmin=0, vmax=0.14)
    ...
    m = ax3.imshow(np.flip(syn_theory_data['mean_alpha'], axis=0), cmap=cmp, origin='lower', vmin=0,
                   vmax=np.max(syn_theory_data['mean_alpha']))
    cb = plt.colorbar(m, ax=ax3)
    cb.ax.tick_params(labelsize=pu.fs1, width=0.5, pad=2)
    cb.set_ticks([0, 0.2, 0.4])
    cb.set_ticklabels(['0.0', '0.2', '0.4'])

    # info content, I
    m = ax4.imshow(np.flip(syn_theory_data['mean_alpha'], axis=0) * missinfo(np.flip(syn_theory_data['m'], axis=0)),
                   cmap=cmp, origin='lower', vmin=0,
                   vmax=np.max(syn_theory_data['mean_alpha'] * missinfo(syn_theory_data['mean_m'])))
    ...
    # formatting
    ylabs = [r'Syn gating, $1$$-$$c$', r'Syn gating, $1$$-$$c$', r'Syn gating, $1$$-$$c$', r'Syn gating, $1$$-$$c$']
    ...
    s = [1, 2, 10, 100]
    svals = syn_theory_data['svals']
    cvals = np.insert(syn_theory_data['cvals'], 0, 0)
    for i in range(len(s)):
        idx = np.where(svals == s[i])[0][0]
        ax5.plot(1 - cvals, np.insert(syn_theory_data['mean_m'][:, idx], 0, -1), linewidth=0.5, c=pu.syn_clrs4[i])
        ax6.plot(1 - cvals, np.insert(syn_theory_data['mean_alpha_cxt'][:, idx], 0, -1), linewidth=0.5,
                 c=pu.syn_clrs4[i])
        ax7.plot(1 - cvals, np.insert(syn_theory_data['mean_alpha'][:, idx], 0, -1), linewidth=0.5, c=pu.syn_clrs4[i])
        ax8.plot(1 - cvals, np.insert(syn_theory_data['mean_alpha'][:, idx]
                                      * missinfo(syn_theory_data['mean_m'][:, idx]), 0, -1),
                 linewidth=0.5, c=pu.syn_clrs4[i])

    # plot numerics as points
    cvals = numerics_data['cvals']
    svals = numerics_data['svals']
    a_idxs = [np.where(np.abs(syn_theory_data['cvals'] - cvals[i]) < 1e-5)[0][0] for i in range(len(cvals))]
    for i in range(len(s)):
        idx = np.where(svals == s[i])[0][0]
        m_idx = np.where(syn_theory_data['svals'] == s[i])[0][0]
        ax6.plot(1 - cvals, numerics_data['alpha_cxt'][:, idx], '.', linewidth=0.5, c=pu.syn_clrs4[i], markersize=2)
        ax7.plot(1 - cvals, numerics_data['alpha'][:, idx], '.', linewidth=0.5, c=pu.syn_clrs4[i], markersize=2)
        ax8.plot(1 - cvals, numerics_data['alpha'][:, idx] * missinfo(syn_theory_data['m'][a_idxs, m_idx]), '.',
                 linewidth=0.5, c=pu.syn_clrs4[i], markersize=2)

    # formatting
    ax5.legend(('$s$=1', '$s$=2', '$s$=10', '$s$=100'), handlelength=1.5,
               fontsize=pu.fs1, frameon=False, loc='upper right', bbox_to_anchor=(0.7, 0.67))
    ax6.text(0.8, 0.12, 'sim', fontsize=pu.fs1)  # 0.13
    ax6.text(0.8, 0.1, 'theory', fontsize=pu.fs1)
    ax6.plot([0.68], [0.125], '.', c='black', markersize=2)
    ax6.plot([0.615, 0.725], [0.105, 0.105], '-', c='black', linewidth=0.5)
    ...
    ax6.set_ylim([0, 0.15])
    ax7.set_yticks([0., 0.2, 0.4])
    ax7.set_yticklabels(['0.0', '0.2', '0.4'], fontsize=pu.fs1)
    ax7.set_ylim([0, 0.45])
    ...
    if save_pdf:
        f.savefig(f"{pu.fig_path}/prx_fig_04.pdf", bbox_inches='tight', pad_inches=0)
    if show_plot:
        plt.show()
