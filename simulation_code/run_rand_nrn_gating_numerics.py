...
rand_cross = False
noise = 0.0
overlap_path = '../data/numerics/overlaps/random_neuronal_gating/'
linreg = LinearRegression()


def run_batch_random_neuronal_recall(avals, svals, alpha_levels):

    for i, a in tqdm(enumerate(avals), desc='a'):
        for j, s in tqdm(enumerate(svals), leave=False, desc='s'):
            ...
            for ii, a0 in enumerate(alpha_levels):
                ...
                with open(overlap_path + f'Ncxt{Ncxt}/Ncxt{Ncxt}_a{int(100*a)}_s{int(s)}'
                          f'_ntrl{n_trials}_alevel{int(1000*a0)}_nlevel{int(1000*noise)}.pkl', 'wb') as tmp_file:
                    # noinspection PyTypeChecker
                    pickle.dump(sim_data, tmp_file)


def estimate_capacity_random_neuronal_gating(avals, svals, alpha_levels, visualize_results=False):
    ...
    for i, a in tqdm(enumerate(avals), desc='a'):
        for j, s in tqdm(enumerate(svals), leave=False, desc='s'):
            ...
            for ii, a0 in enumerate(alpha_levels):
                ...
                # load data
                with open(overlap_path + f'Ncxt{Ncxt}/Ncxt{Ncxt}_a{int(100 * a)}'
    ...
    with open(f'../data/numerics/capacity/Ncxt{Ncxt}_rand_nrn_gating_capacity.pkl', 'wb') as file:
        # noinspection PyTypeChecker
        pickle.dump(capacity_data, file)


def estimate_stability_random_neuronal_gating(avals, svals):
    ...
    for i, a in tqdm(enumerate(avals), desc='a'):
        for j, s in tqdm(enumerate(svals), leave=False, desc='s'):
            ...
            net_prms = {'N': N, 'a': a, 'c': c, 's': s, 'syn_ref': syn_ref}

            theory_data = tf.get_random_gating_theory(s, a, c)
            p = int(np.round(a * N * theory_data['alpha_cxt']))
    ...
    with open(f'../data/numerics/stability/N{N}_rand_nrn_gating_stability.pkl', 'wb') as file:
        # noinspection PyTypeChecker
...
def main():
    ...
    run_batch_random_neuronal_recall(avals, svals, alpha_levels)
    estimate_capacity_random_neuronal_gating(avals, svals, alpha_levels)
    estimate_stability_random_neuronal_gating(avals, svals)
