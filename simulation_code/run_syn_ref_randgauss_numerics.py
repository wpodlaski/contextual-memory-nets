...
rand_cross = False
noise = 0.0
overlap_path = '../data/numerics/overlaps/synaptic_refinement_randgauss/'
linreg = LinearRegression()


def run_batch_synaptic_refinement_randgauss_recall(avals, svals, alpha_levels):

    theory_data = {'alpha_cxt': tf.alpha_Delta(np.pi ** 2 / 2),
                   'm': tf.m_Delta(np.pi ** 2 / 2),
                   'kappa_acc': np.sqrt(2. / np.pi ** 2 / tf.alpha_Delta(np.pi ** 2 / 2)),
                   'kappa_inacc': 0.0}
    ...
    for i, a in tqdm(enumerate(avals)):
        for j, s in tqdm(enumerate(svals), leave=False):
            ...
            for ii, a0 in enumerate(alpha_levels):
                ...
                with open(f'data/numerics/overlaps/synaptic_refinement_randgauss/Ncxt{Ncxt}/'
                          f'Ncxt{Ncxt}_ntrl{n_trials}_alevel{int(1000 * a0)}_'
                          f'nlevel{int(1000 * noise)}.pkl', 'wb') as tmp_file:
                    # noinspection PyTypeChecker
                    pickle.dump(sim_data, tmp_file)


def estimate_capacity_synaptic_refinement_randgauss(avals, svals, alpha_levels, visualize_results=False):
    ...
    with open(f'data/numerics/capacity/Ncxt{Ncxt}_syn_ref_randgauss_capacity.pkl', 'wb') as tmp_file:
        # noinspection PyTypeChecker
        pickle.dump(capacity_data, tmp_file)


def estimate_stability_synaptic_refinement_randgauss(avals, svals):
    ...
    for i, a in tqdm(enumerate(avals)):
        for j, s in tqdm(enumerate(svals), leave=False):
            ...
            net_prms = {'N': Ncxt, 'a': a, 'c': 1.0, 's': s, 'syn_ref': syn_ref, 'randgauss': True}
    ...
    with open(f"data/numerics/stability/N{net_prms['N']}_syn_ref_randgauss_stability.pkl", "wb") as tmp_file:
        # noinspection PyTypeChecker
...
def main():
    ...
    run_batch_synaptic_refinement_randgauss_recall(avals, svals, alpha_levels)
    estimate_capacity_synaptic_refinement_randgauss(avals, svals, alpha_levels)
    estimate_stability_synaptic_refinement_randgauss(avals, svals)
