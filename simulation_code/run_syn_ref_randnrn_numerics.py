...
rand_cross = False
noise = 0.0
overlap_path = '../data/numerics/overlaps/synaptic_refinement_randnrn/'
linreg = LinearRegression()


def run_batch_synaptic_refinement_random_neuronal_recall(avals, svals, alpha_levels):
    ...
    for i, a in tqdm(enumerate(avals)):
        for j, s in tqdm(enumerate(svals), leave=False):
            ...
            for ii, a0 in enumerate(alpha_levels):
                ...
                with open(f'data/numerics/overlaps/synaptic_refinement_randnrn/Ncxt{Ncxt}/'
                          f'Ncxt{Ncxt}_a{int(100*a)}_s{int(s)}'
                          f'_ntrl{n_trials}_alevel{int(1000*a0)}_nlevel{int(1000*noise)}.pkl', 'wb') as tmp_file:
                    # noinspection PyTypeChecker
                    pickle.dump(sim_data, tmp_file)


def estimate_capacity_synaptic_refinement_random_neuronal_gating(avals, svals, alpha_levels, visualize_results=False):
    ...
    for i, a in enumerate(avals):
        for j, s in enumerate(svals):
            ...
            for ii, a0 in enumerate(alpha_levels):
                ...
                # load data
                with open(f'data/numerics/overlaps/synaptic_refinement_randnrn/Ncxt{Ncxt}/'
                          f'Ncxt{Ncxt}_a{int(100 * a)}_s{int(s)}_ntrl{n_trials}_alevel{int(1000 * a0)}_'
    ...
    with open(f'data/numerics/capacity/Ncxt{Ncxt}_syn_ref_randnrn_capacity.pkl', 'wb') as tmp_file:
        # noinspection PyTypeChecker
        pickle.dump(capacity_data, tmp_file)


def estimate_stability_synaptic_refinement_random_neuronal_gating(avals, svals):
    ...
    for i, a in tqdm(enumerate(avals)):
        for j, s in tqdm(enumerate(svals), leave=False):
            ...
            p = int(np.round(a * net_prms['N'] * theory_data['mean_alpha_cxt']))
    ...
    with open(f"data/numerics/stability/N{net_prms['Ncxt']}_syn_ref_randnrn_stability.pkl", "wb") as file:
        # noinspection PyTypeChecker
...
def main():
    ...
    avals = np.array([0.05, 0.15, 0.25, 0.35, 0.45, 0.65, 0.85])
    svals = np.array([1, 2, 5, 10, 50, 100])
    alpha_levels = np.array([-0.02, -0.01, -0.005, -0.001, 0.0, 0.001, 0.005, 0.01])

    run_batch_synaptic_refinement_random_neuronal_recall(avals, svals, alpha_levels)
    estimate_capacity_synaptic_refinement_random_neuronal_gating(avals, svals, alpha_levels)
    estimate_stability_synaptic_refinement_random_neuronal_gating(avals, svals)
